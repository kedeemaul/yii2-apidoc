...
namespace yii2docs\apidoc\models;
...
class ActionModel extends Component
{
	...
	public function init()
	{
        ...
        if (preg_match('/@create\s*(.*)\n/', $comment, $matches) && !empty($matches[1])) {
            $this->_create = trim($matches[1], "\t\n\r\0\x0B");
        } else {
            $this->_create = "";
        }

        if (preg_match('/@request\s*(.*)\n/', $comment, $matches) && !empty($matches[1])) {
            $this->_request = trim($matches[1], "\t\n\r\0\x0B");
        } else {
            $this->_request = "参数名以*号开头表示必填";
        }
        ...
        if (preg_match('/@response\s*(.*)\n/', $comment, $matches) && !empty($matches[1])) {
            $this->_response = trim($matches[1], "\t\n\r\0\x0B");
        } else {
            $this->_response = "";
        }
        if (preg_match_all('/@return\s*(.*)\n/', $comment, $matches) && !empty($matches[1])) {
            foreach ($matches[1] as $match) {
                $info = preg_split("/[\s]+/", $match, 3);
                $returns = [
                    'type' => isset($info[0]) ? $info[0] : '',
    ...
    public function getRequesttips()
    {
